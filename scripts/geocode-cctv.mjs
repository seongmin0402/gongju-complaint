/**
 * CCTV 주소 → 좌표 일괄 변환 스크립트
 * node scripts/geocode-cctv.mjs
 */
...
function buildQuery(r) {
  ...
  if (!ins) return null;
  // 지번주소가 이미 읍면동으로 시작하면 바로 붙임, 아니면 관할 앞에 추가
  const hasAdminUnit = /^(유구읍|이인면|탄천면|계룡면|반포면|의당면|정안면|우성면|사곡면|신풍면|중학동|웅진동|금학동|옥룡동|신관동|월송동)/.test(ins);
  const addr = hasAdminUnit ? `충청남도 공주시 ${ins}` : `충청남도 공주시 ${r.jurisdiction} ${ins}`;
  return addr;
}

async function geocode(query) {
  const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=3&countrycodes=kr&accept-language=ko`;
  const res = await fetch(url, {
    headers: { 'User-Agent': 'GongjuCctvGeocoder/1.0', Accept: 'application/json' },
  });
...
for (let i = 0; i < RAW.length; i++) {
  ...
  if (!q) {
    console.log(`[${r.row_no}] SKIP (no query)`);
    results.push({ ...r, lat: null, lng: null });
    continue;
  }
  const coords = await geocode(q);
  if (coords) {
    console.log(`[${r.row_no}] OK  ${coords.lat.toFixed(5)}, ${coords.lng.toFixed(5)}  ← ${q}`);
    results.push({ ...r, lat: coords.lat, lng: coords.lng });
  } else {
    console.log(`[${r.row_no}] FAIL  ← ${q}`);
    results.push({ ...r, lat: null, lng: null });
  }
...
if (fail.length) console.log('실패:', fail.map(r => r.row_no).join(', '));
...
writeFileSync(outPath, `// 자동 생성 — node scripts/geocode-cctv.mjs\nexport interface CctvCoord {\n  row_no: number;\n  jurisdiction: string;\n  install_location: string;\n  note: string;\n  lat: number | null;\n  lng: number | null;\n}\n\nexport const CCTV_COORDS: CctvCoord[] = [\n${lines.join('\n')}\n];\n`);
console.log(`\n저장: lib/cctv-coords.ts`);
